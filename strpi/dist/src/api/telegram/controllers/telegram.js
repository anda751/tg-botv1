...
async function handleCallbackQuery(query, strapi) {
    ...
    const managers = await strapi.entityService.findMany('plugin::users-permissions.user', { filters: { telegram_id: String(from.id), role_app: 'manager' }, limit: 1 });
    if (!managers.length) {
...
async function handleMessage(message, strapi) {
    ...
    if (pending) {
        ...
        const managers = await strapi.entityService.findMany('plugin::users-permissions.user', { filters: { telegram_id: userId, role_app: 'manager' }, limit: 1 });
        if (managers.length) {
    ...
    if (text === '/start') {
        await sendMessage(botToken, chat.id, `👋 สวัสดีครับ ${from.first_name}\nใช้ Mini App เพื่อจัดการงานได้เลย`);
    }
}
async function approveTask(taskId, manager, strapi, botToken, chatId, originalMessage) {
...
async function rejectTask(taskId, reason, manager, strapi, botToken, chatId) {
    const task = await strapi.entityService.findOne('api::task.task', taskId, {
        populate: ['current_owner'],
    });
    if (!task) {
...
async function approveHandover(handoverId, manager, strapi, botToken, chatId, originalMessage) {
    const handover = await strapi.entityService.findOne('api::handover-request.handover-request', handoverId, { populate: ['task', 'requested_by', 'picked_up_by'] });
...
async function approveUserFromTelegram(userId, manager, strapi, botToken, chatId, originalMessage) {
...
async function rejectUserFromTelegram(userId, reason, manager, strapi, botToken, chatId) {
    const target = await strapi.entityService.findOne('plugin::users-permissions.user', userId);
    if (!target) {
        await sendMessage(botToken, chatId, '❌ ไม่พบผู้ใช้นี้');
        return;
