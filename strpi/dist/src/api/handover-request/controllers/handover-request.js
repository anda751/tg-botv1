...
exports.default = strapi_1.factories.createCoreController('api::handover-request.handover-request', ({ strapi }) => ({
    ...
    async handover(ctx) {
        ...
        if (task.status_task !== 'in_progress')
            return ctx.badRequest('งานนี้ยังไม่สามารถส่งต่อได้');
    ...
    async pickup(ctx) {
        ...
        if (task.status_task !== 'waiting_pickup')
            return ctx.badRequest('งานนี้ไม่ได้อยู่ในสถานะรอรับต่อ');
        if (task.current_owner.id === user.id)
            return ctx.badRequest('คุณเป็นเจ้าของงานเดิมอยู่แล้ว');
        ...
        if (!handovers.length)
            return ctx.notFound('ไม่พบคำขอส่งต่องาน');
    ...
    async approveHandover(ctx) {
        ...
        if (!handover)
            return ctx.notFound('ไม่พบคำขอ');
        if (handover.status_handover !== 'pending')
            return ctx.badRequest('คำขอนี้ไม่ได้อยู่ในสถานะรออนุมัติ');
        if (!handover.picked_up_by)
            return ctx.badRequest('ยังไม่มีผู้ขอรับงานนี้');
    ...
    async cancelHandover(ctx) {
        ...
        if (!handover)
            return ctx.notFound('ไม่พบคำขอ');
        if (handover.requested_by.id !== user.id)
            return ctx.forbidden('คุณไม่ใช่เจ้าของคำขอนี้');
        if (handover.status_handover !== 'pending')
            return ctx.badRequest('คำขอนี้ไม่สามารถยกเลิกได้');
        await strapi.entityService.update('api::handover-request.handover-request', id, {
