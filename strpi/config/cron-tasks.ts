  ...
  '*/5 * * * *': {
    task: async ({ strapi }) => {
      try {
        ...
        const expiredHandovers = await strapi.entityService.findMany(
        ...
        for (const handover of expiredHandovers) {
          try {
            ...
            if (handover.requested_by) {
              await strapi
                ...
                .notifyStaff({
                  ...
                  message:
                    `⏰ คำขอรับงาน *${handover.task.name}* หมดเวลาแล้ว (30 นาที)\n` +
                    `งานยังรอคนรับอยู่`,
                });
            }
            ...
            await strapi
              ...
              .notifyGroup({
                message:
                  `⏰ คำขอรับงาน *${handover.task.name}* หมดเวลา\n` +
                  `งานยังรอคนรับอยู่`,
              });

            strapi.log.info(
              `[Cron] Handover timeout: task ${handover.task.id}`,
            );
          } catch (err) {
            strapi.log.error(
              `[Cron] Error processing handover ${handover.id}`,
              err,
            );
          }
        }
  ...
  '0 8 * * *': {
    task: async ({ strapi }) => {
      try {
        ...
        const pendingTasks = await strapi.entityService.findMany(
          'api::task.task',
          {
            filters: {
              status_task: {
        ...
        const inProgress = pendingTasks.filter(
          (t) => t.status_task === 'in_progress',
        );

        const waitingPickup = pendingTasks.filter(
          (t) => t.status_task === 'waiting_pickup',
        );

        const underReview = pendingTasks.filter(
          (t) => t.status_task === 'under_review',
        );
        ...
        if (inProgress.length) {
          ...
          inProgress.forEach((t) => {
            message +=
              `• ${t.name} → ` +
              `${t.current_owner?.username ?? 'ไม่มีผู้รับผิดชอบ'}\n`;
          });
        ...
        if (underReview.length) {
          ...
          underReview.forEach((t) => {
            message +=
              `• ${t.name} → ` +
              `ส่งโดย ${t.current_owner?.username ?? '-'}\n`;
          });
        }
        ...
        strapi.log.info(
          `[Cron] Morning summary sent: ${pendingTasks.length} tasks`,
        );
      } catch (err) {
      ...
      try {
        ...
        for (const project of projects) {
          try {
            ...
            strapi.log.error(
              `[Cron] Error notifying project ${project.id}`,
              err,
            );
          }
        }
         ...
         * ===== Overdue =====
         */
        const overdueProjects = await strapi.entityService.findMany(
          'api::project.project',
          {
            filters: {
              deadline: { $lt: now },
        ...
        for (const project of overdueProjects) {
          try {
            const overdueTasks = await strapi.entityService.findMany(
              'api::task.task',
              {
                filters: {
                  project: {
            ...
            strapi.log.error(
              `[Cron] Error overdue project ${project.id}`,
              err,
            );
          }
        }
