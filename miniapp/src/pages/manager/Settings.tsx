    ...
    <div className="min-h-screen bg-slate-950 flex flex-col">
      <div className="bg-slate-900 border-b border-slate-800 px-4 pt-6 pb-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">แก้ไขข้อมูลหัวหน้า รหัสผ่าน และ Telegram</p>
          </div>
          <button
            onClick={loadProfile}
      ...
      <div className="flex-1 px-4 py-5 space-y-4">
        {loading ? (
          ...
          <>
            {error && (
              <NoticeBox tone="red" title="บันทึกข้อมูลไม่สำเร็จ" message={error} />
            )}
            {success && (
              <NoticeBox tone="green" title="บันทึกสำเร็จ" message={success} />
            )}
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-4 space-y-3">
              ...
              <p className="text-xs text-slate-500">
                หากยังไม่ต้องการเปลี่ยนรหัสผ่าน สามารถเว้นสามช่องนี้ว่างไว้ได้
              </p>
              <button
                onClick={handleSave}
                disabled={saving}
